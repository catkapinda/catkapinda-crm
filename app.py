...
def restaurants_tab(conn: sqlite3.Connection) -> None:
    ...
    personnel_display = format_display_df(
        df,
        currency_cols=["monthly_fixed_cost", "accounting_revenue", "accountant_cost", "company_setup_revenue", "company_setup_cost"],
        rename_map={
    ...
    st.dataframe(personnel_display, width='stretch', hide_index=True)
    ...
    if not df.empty:
        ...
        current_active = int(df.loc[df['id'] == selected_id, 'active'].iloc[0])
...
def deductions_tab(conn: sqlite3.Connection) -> None:
    ...
    with st.form("deduction_form", clear_on_submit=True):
        ...
        ded_type = c3.selectbox("Kesinti türü", ["Motor kira", "Yakıt", "HGS", "İdari ceza", "Hasar", "Zimmet", "Muhasebe Ücreti", "Şirket Açılış Ücreti", "Diğer"])
    ...
    df = fetch_df(conn, """
        SELECT d.deduction_date, p.full_name AS personel, d.deduction_type, d.amount, d.notes
        FROM deductions d
        JOIN personnel p ON p.id = d.personnel_id
        ORDER BY d.deduction_date DESC, p.full_name
    """)
    ...
    st.dataframe(deductions_display_df, width='stretch', hide_index=True)
...
def reports_tab(conn: sqlite3.Connection) -> None:
    ...
    accounting_rev = float(personnel_df["accounting_revenue"].fillna(0).sum()) if "accounting_revenue" in personnel_df.columns else 0.0
    accountant_cost_total = float(personnel_df["accountant_cost"].fillna(0).sum()) if "accountant_cost" in personnel_df.columns else 0.0
    setup_rev = float(personnel_df["company_setup_revenue"].fillna(0).sum()) if "company_setup_revenue" in personnel_df.columns else 0.0
    setup_cost = float(personnel_df["company_setup_cost"].fillna(0).sum()) if "company_setup_cost" in personnel_df.columns else 0.0
