...
def test_payroll_dashboard_adds_extra_day_bonus_for_fixed_monthly_courier_hours_over_260():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        ...
        VALUES ('2026-03-21', 11, 1, 1, 270, 0)
        """
    )
    raw_conn.commit()
    ...
    assert payload.summary is not None
    assert round(payload.summary.gross_payroll, 2) == 76053.33
    assert round(payload.entries[0].gross_pay, 2) == 76053.33
    ...
    assert ("Ek Mesai Hakedişi", 2453.3333333333335) == document_payload.earning_items[0]
    html = _build_payroll_document_html(document_payload)
    assert "Ek Mesai Hakedişi" in html
    assert "2.453,33 ₺" in html


def test_payroll_document_shows_extra_day_bonus_for_fixed_kurye_on_fixed_monthly_brand():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        """
        INSERT INTO personnel (id, full_name, person_code, role, status, cost_model, monthly_fixed_cost)
        VALUES (1, 'Seyfullah Aksu', 'CK-K40', 'Kurye', 'Aktif', 'fixed_kurye', 0)
        """
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (11, 'SC Petshop', 'Merkez')")
    raw_conn.execute(
    ...
    assert ("Ek Mesai Hakedişi", 2453.3333333333335) == document_payload.earning_items[0]
    assert round(document_payload.gross_pay, 2) == 76053.33
...
def test_payroll_dashboard_uses_fixed_monthly_brand_pay_for_standard_courier_working_only_fixed_brand():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        """
        INSERT INTO personnel (id, full_name, person_code, role, status, cost_model, monthly_fixed_cost)
        VALUES (1, 'Hayrettin İçli', 'CK-K41', 'Kurye', 'Aktif', 'standard_courier', 0)
        """
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (11, 'Sushi Inn', 'Merkez')")
    raw_conn.execute(
        """
        INSERT INTO daily_entries (
            entry_date,
...
def test_payroll_document_uses_fixed_invoice_amount_with_holiday_bonus_for_team_chief():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        """
        INSERT INTO personnel (id, full_name, person_code, role, status, cost_model, monthly_fixed_cost)
        VALUES (1, 'Recep', 'CK-RTS01', 'Restoran Takım Şefi', 'Aktif', 'fixed_restoran_takim_sefi', 76853.33)
        """
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (10, 'Quick China', 'Ataşehir')")
    raw_conn.executemany(
        ...
        """,
        [
            (f"2026-03-{day:02d}", 10, 1, 1, 11, 0, 84500)
            for day in range(1, 31)
        ],
    )
    raw_conn.commit()
    ...
    payload = build_payroll_dashboard(CompatConnection(raw_conn, "sqlite"), selected_month="2026-03")
    assert round(payload.entries[0].tevkifat_amount, 2) == 3605.33
    ...
    assert round(document_payload.invoice_base_amount, 2) == 90133.33
    assert round(document_payload.invoice_vat_amount, 2) == 18026.67
    assert round(document_payload.tevkifat_amount, 2) == 3605.33
    assert any(
        item[0] == "Tevkifat (Yasal yükümlülük kesintisi)" and round(item[1], 2) == 3605.33
        for item in document_payload.deduction_items
    )


def test_payroll_document_uses_fixed_monthly_brand_invoice_with_overtime_day():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        """
        INSERT INTO personnel (id, full_name, person_code, role, status, cost_model, monthly_fixed_cost)
        VALUES (1, 'Seyfullah', 'CK-K44', 'Kurye', 'Aktif', 'fixed_kurye', 73600)
        """
    )
    raw_conn.execute(
        """
        INSERT INTO restaurants (id, brand, branch, pricing_model)
        VALUES (10, 'SC Petshop', 'Merkez', 'fixed_monthly')
        """
    )
    raw_conn.execute(
        """
        ...
        VALUES ('2026-03-15', 10, 1, 1, 270, 0, 79800)
        """
    )
    raw_conn.commit()
    ...
    assert round(document_payload.invoice_base_amount, 2) == 82460.0
    assert round(document_payload.invoice_vat_amount, 2) == 16492.0
    assert round(document_payload.tevkifat_amount, 2) == 3298.4


def test_payroll_document_uses_fixed_monthly_brand_daily_invoice_for_support_assignment():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.executemany(
        ...
        """,
        [
            (1, 'Seyfullah', 'CK-K44', 'Kurye', 'Aktif', 'fixed_kurye', 73600),
            (2, 'Erkan Çelik', 'CK-K45', 'Kaptan', 'Aktif', 'standard_courier', 0),
        ],
    )
    raw_conn.execute(
        """
    ...
    raw_conn.executemany(
        ...
        """,
        [
            ('2026-03-15', 10, 1, 1, 260, 0, 79800),
            ('2026-03-16', 10, 1, 2, 10, 0, 79800),
        ],
    )
    raw_conn.commit()
    ...
    assert round(document_payload.invoice_base_amount, 2) == 2660.0
    assert round(document_payload.invoice_vat_amount, 2) == 532.0
    assert round(document_payload.tevkifat_amount, 2) == 0.0
...
def test_payroll_uses_vehicle_history_for_partial_month_motor_rental_deduction():
    ...
    raw_conn.execute(
        ...
        INSERT INTO personnel (
            id, full_name, person_code, role, status, cost_model, monthly_fixed_cost,
            start_date, vehicle_type, motor_rental, motor_purchase, motor_rental_monthly_amount
        )
        VALUES (1, 'Cüneyt Mehder', 'CK-K55', 'Kurye', 'Aktif', 'fixed_monthly', 30000, '2026-03-19', 'Kendi Motoru', 'Hayır', 'Hayır', 13000)
        """
    )
    raw_conn.executemany(
        """
        ...
        """,
        [
            (1, 1, 'Kendi Motoru', 'Hayır', 13000, 'Hayır', None, 0, 0, 0, '2026-03-19', '2026-03-19 09:00:00', 'Baslangic'),
            (2, 1, 'Çat Kapında', 'Evet', 13000, 'Hayır', None, 0, 0, 0, '2026-03-21', '2026-03-21 09:00:00', 'Motor kiraladi'),
            (3, 1, 'Kendi Motoru', 'Hayır', 13000, 'Hayır', None, 0, 0, 0, '2026-03-24', '2026-03-24 09:00:00', 'Kendi motoruna dondu'),
        ],
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (10, 'Quick China', 'Ataşehir')")
    raw_conn.execute(
    ...
    entry = payload.entries[0]
    assert any(item.label == "Motor Kirası" and round(item.amount, 2) == 1300.00 for item in entry.deduction_items)
...
def test_payroll_history_sale_uses_purchase_start_date_when_sale_row_was_entered_late():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        ...
        VALUES (
            1, 'Seyfullah Aksu', 'CK-K35', 'Kurye', 'Aktif', 'fixed_kurye', 0, '2025-08-02',
            'Çat Kapında', 'Hayır', 'Evet', 13000, '2025-11-01', 12, 135000, 11250
        )
        """
    )
    raw_conn.executemany(
        ...
        """,
        [
            (36, 1, 'Çat Kapında', 'Evet', 13000, 'Hayır', None, 0, 0, 0, '2025-08-02', '2025-08-02 09:00:00', 'Sistem: Başlangıç araç kaydı'),
            (116, 1, 'Çat Kapında', 'Hayır', 13000, 'Evet', '2025-11-01', 12, 135000, 11250, '2026-05-02', '2026-05-02 09:00:00', 'Sistem: Personel kartından motor geçişi'),
        ],
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (11, 'SC Petshop', 'Merkez')")
    raw_conn.execute(
    ...
    assert all(item.label != "Motor Kirası" for item in entry.deduction_items)
    assert any(item.label == "Motor Satış Taksiti" and round(item.amount, 2) == 11250.0 for item in entry.deduction_items)
    ...
    assert all(item[0] != "Motor Kirası" for item in document_payload.deduction_items)
    assert any(item[0] == "Motor Satış Taksiti" and round(item[1], 2) == 11250.0 for item in document_payload.deduction_items)


def test_payroll_history_rental_uses_motor_start_date_when_rental_row_was_entered_late():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        ...
        VALUES (
            1, 'Ali Ekber Çiftçi', 'CK-K55', 'Kurye', 'Aktif', 'standard_courier', 0, '2026-02-09',
            'Çat Kapında', 'Evet', 'Hayır', 13000, '2026-03-01', 0, 0, 0
        )
        """
    )
    raw_conn.execute(
        ...
        VALUES (
            ...
            '2026-04-12',
            '2026-04-12 09:00:00',
            'Sistem: Başlangıç araç kaydı'
        )
        """
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (11, 'Quick China', 'Ataşehir')")
    raw_conn.execute(
        """
        INSERT INTO daily_entries (
            entry_date,
        ...
        VALUES ('2026-03-21', 11, 1, 1, 120, 140)
        """
    )
    raw_conn.commit()
    ...
    assert any(item.label == "Motor Kirası" and round(item.amount, 2) == 13000.0 for item in entry.deduction_items)
    ...
    assert any(item[0] == "Motor Kirası" and round(item[1], 2) == 13000.0 for item in document_payload.deduction_items)


def test_payroll_prorates_rental_from_person_card_motor_start_date_without_history():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        ...
        VALUES (
            1, 'Sude Erarslan', 'CK-K91', 'Kurye', 'Aktif', 'standard_courier', 0, '2026-03-31',
            'Çat Kapında', 'Evet', 'Hayır', 13000, '2026-03-31', 0, 0, 0
        )
        """
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (11, 'Quick China', 'Ataşehir')")
    raw_conn.execute(
        ...
        VALUES ('2026-03-31', 11, 1, 1, 10, 12)
        """
    )
    raw_conn.commit()
    ...
    assert any(item.label == "Motor Kirası" and round(item.amount, 2) == 433.33 for item in entry.deduction_items)


def test_payroll_falls_back_to_person_card_rental_when_history_has_no_rental_for_month():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
