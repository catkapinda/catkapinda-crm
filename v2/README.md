...
- Backend:
  ...
  - frontend `.env.local` dosyasini local API hedefine gore yeniden yazmak icin:
    - `python v2/scripts/local_v2_doctor.py --write-frontend-env`
...
- `pilot_day_zero.py` artik verify raporunu iki asamada yazar; boylece bundle icindeki `pilot-day-zero-verify.*` dosyalari final kitin kendisini anlatir
...
- eger kit smoke ile uretildiyse verify artik:
  ...
  - `pilot-smoke-live.md` raporunun `pilot-smoke-live.json` ile, metadata, baslik/yapi satirlari, karar satirlari ve check tablosu, satir sirasi, beklenmeyen/tekrarlanan tablo satirlari, stale summary bullet'lari, beklenmeyen yapi satirlari ve serbest stale satirlar dahil, birebir uyumlu kalip kalmadigini
  - `pilot-smoke-live.json` icindeki passed/failed/overall ve failing_checks alanlarinin result listesiyle, sirasi dahil, tutarli kalip kalmadigini
  - `pilot-smoke-live.json` icindeki cekirdek decision alanlarinin (`status`, `headline`, `primary_blocker`, `recommended_next_step`) result listesiyle tutarli kalip kalmadigini
