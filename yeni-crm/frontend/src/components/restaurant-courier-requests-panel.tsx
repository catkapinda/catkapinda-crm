...
export function RestaurantCourierRequestsPanel({
  ...
  async function markFulfilled(id: number, fulfilledAt: string) {
    try {
  ...
  async function updateFulfilledDate(id: number, fulfilledAt: string) {
    try {
      await updateRestaurantCourierRequest(restaurantId, id, {
        fulfilled_at: fulfilledAt,
      });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {openItems.map((req) => (
                    <OpenRequestCard
                      key={req.id}
                      req={req}
                      onMarkFulfilled={(date) => markFulfilled(req.id, date)}
                    ...
                    <tbody>
                      {visibleHistory.map((req) => (
                        <HistoryRow
                          ...
                          onMarkFulfilled={(date) => markFulfilled(req.id, date)}
                          onUpdateFulfilledDate={(date) => updateFulfilledDate(req.id, date)}
                        />
                      ))}
                    </tbody>
                  </table>
...
function OpenRequestCard({
  ...
  onMarkFulfilled: (date: string) => void;
  onCancel: () => void;
}) {
  const meta = TYPE_META[req.change_type];
  const Icon = meta.icon;
  // İki aşama: ilk butonda 'Karşılandı' → tıklayınca tarih input formu açılır
  const [fulfilling, setFulfilling] = useState(false);
  const [fulfilledDate, setFulfilledDate] = useState<string>(todayIso());
  ...
  return (
    <div className={`relative rounded-xl ring-1 ${meta.ring} p-3 hover:shadow-md transition-all`}>
      ...
      {fulfilling ? (
        // Karşılanma tarihi seç + onay
        <div className="rounded-lg bg-emerald-50/60 ring-1 ring-emerald-200 p-2.5 space-y-2 animate-slide-down">
          <label className="block text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-800">
            Karşılanma tarihi
          </label>
          <input
            type="date"
            ...
            autoFocus
            className="w-full px-2.5 py-1.5 rounded-md border border-emerald-300 bg-white text-[12.5px] focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition"
          />
          <div className="flex gap-2">
            <button
              onClick={() => onMarkFulfilled(fulfilledDate)}
              disabled={!fulfilledDate}
              className="flex-1 px-2.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[11.5px] font-bold transition inline-flex items-center justify-center gap-1 disabled:opacity-50"
            >
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg bg-white border border-emerald-200 hover:bg-emerald-50 text-text-2 text-[11.5px] font-semibold transition"
            >
              Vazgeç
            </button>
          </div>
...
function HistoryRow({
  req, onDelete, onMarkFulfilled, onUpdateFulfilledDate,
}: {
  req: RestaurantCourierRequest;
  onDelete: () => void;
  onMarkFulfilled: (date: string) => void;
  onUpdateFulfilledDate: (date: string) => void;
}) {
  const meta = TYPE_META[req.change_type];
  const Icon = meta.icon;
  const statusMeta = {
  ...
  // Bekleyen satırda 'Karşılandı' tıklanınca tarih girme moduna geç
  const [fulfilling, setFulfilling] = useState(false);
  const [fulfilledDate, setFulfilledDate] = useState<string>(
    req.fulfilled_at ?? todayIso(),
  );
  // Karşılanmış satırda hücreye tıklanınca düzenle
  const [editingDate, setEditingDate] = useState(false);
  const [editDate, setEditDate] = useState<string>(req.fulfilled_at ?? todayIso());
  ...
  return (
    <tr className="border-t border-border hover:bg-bg-surface2/40 transition">
      ...
      <td className="px-3 py-2 text-[12px]">
        {fulfilling ? (
          // Bekleyen → karşılandı işaretleme: tarih + onay/vazgeç
          <div className="inline-flex items-center gap-1">
            <input
              ...
              onChange={(e) => setFulfilledDate(e.target.value)}
              autoFocus
              className="px-1.5 py-0.5 rounded border border-emerald-300 text-[11.5px] focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30"
            />
            <button
              onClick={() => {
                if (fulfilledDate) onMarkFulfilled(fulfilledDate);
                setFulfilling(false);
              }}
              className="p-0.5 rounded text-emerald-700 hover:bg-emerald-50 transition"
              title="Onayla"
            >
              <CheckCircle2 className="w-3.5 h-3.5" strokeWidth={2.4} />
            </button>
            <button
              onClick={() => setFulfilling(false)}
              className="p-0.5 rounded text-text-3 hover:bg-bg-surface transition"
              title="Vazgeç"
            >
              <X className="w-3.5 h-3.5" strokeWidth={2.4} />
            </button>
          ...
          <div className="inline-flex items-center gap-1">
            <input
              ...
              autoFocus
              className="px-1.5 py-0.5 rounded border border-brand text-[11.5px] focus:outline-none focus:ring-1 focus:ring-brand/30"
            />
            <button
              onClick={() => {
                if (editDate && editDate !== req.fulfilled_at) {
                  onUpdateFulfilledDate(editDate);
                }
                setEditingDate(false);
              }}
              className="p-0.5 rounded text-brand hover:bg-brand-soft transition"
              title="Kaydet"
            >
              <CheckCircle2 className="w-3.5 h-3.5" strokeWidth={2.4} />
            </button>
            <button
              onClick={() => {
                setEditDate(req.fulfilled_at ?? todayIso());
                setEditingDate(false);
              }}
              className="p-0.5 rounded text-text-3 hover:bg-bg-surface transition"
              title="Vazgeç"
          ...
          <button
            ...
            className="text-text-2 tabular-nums hover:text-brand transition cursor-pointer underline-offset-2 hover:underline"
            title="Karşılanma tarihini düzenle"
          >
