...
def restaurants_tab(conn: sqlite3.Connection) -> None:
            ...
            if submitted and brand and branch:
                conn.execute(
                    ...
                    (
                        ...
                        extra_req, extra_req_date.isoformat() if isinstance(extra_req_date, date) else None,
                        reduce_req, reduce_req_date.isoformat() if isinstance(reduce_req_date, date) else None,
                        contact_name, contact_phone, contact_email, tax_office, tax_number,
                        notes
                    ),
                )
    ...
    if selected_row is not None:
        ...
        with st.form("restaurant_edit_form"):
            ...
            submitted_edit = st.form_submit_button("Restoranı güncelle")
            if submitted_edit:
                conn.execute(
                    """
                    UPDATE restaurants
                    ...
                    (
                        ...
                        edit_extra_req, edit_extra_req_date.isoformat() if isinstance(edit_extra_req_date, date) else None,
                        edit_reduce_req, edit_reduce_req_date.isoformat() if isinstance(edit_reduce_req_date, date) else None,
