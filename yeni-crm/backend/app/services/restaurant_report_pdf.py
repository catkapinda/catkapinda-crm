...
def _styles() -> dict[str, ParagraphStyle]:
    ...
    return {
        ...
        "hero_sub": ParagraphStyle(
            "hs", fontName=FONT_REGULAR, fontSize=10,
            textColor=colors.HexColor("#CCDDF5"), leading=14,
        ),
        "hero_doc_label": ParagraphStyle(
            "hdl", fontName=FONT_BOLD, fontSize=7.5,
            textColor=colors.HexColor("#A8C0E8"), leading=10,
            alignment=TA_RIGHT,
        ),
        "hero_doc_val": ParagraphStyle(
            "hdv", fontName=FONT_REGULAR, fontSize=9,
        ...
        "section_eyebrow": ParagraphStyle(
            "se", fontName=FONT_BOLD, fontSize=7,
            textColor=BRAND, leading=10, spaceAfter=2,
        ),
...
def _make_hero(restaurant: dict, period: str, styles: dict) -> Table:
    ...
    right = [
        ...
        Spacer(1, 4),
        Paragraph("DÜZENLEYEN", styles["hero_doc_label"]),
        Paragraph(date.today().strftime("%d.%m.%Y"), styles["hero_doc_val"]),
    ]

    t = Table([[left, right]], colWidths=[120 * mm, 54 * mm], rowHeights=[36 * mm])
...
def _make_kpi_row(metrics: dict, styles: dict) -> Table:
    ...
    cells = [
        _kpi_cell(
            ...
            f"Önceki dönem: {_int(metrics['previous_packages'])}",
        ...
        _kpi_cell(
            ...
            f"Ekosistem ort.: {_money(metrics['ecosystem_cpp'])} ₺",
            styles, value_color=AMBER,
        ),
        _kpi_cell(
            "Ort. Paket/Saat",
            f"{metrics['avg_pph']:.2f}".replace(".", ","),
            f"Ekosistem ort.: {metrics['ecosystem_pph']:.2f}".replace(".", ","),
...
def _make_comparison_box(metrics: dict, styles: dict) -> Table:
    """Bu dönem vs. önceki dönem (paket, churn, verimlilik) tablosu."""
    ...
    rows = [
        ...
        [
            "Churn (turnover)",
            _pct(metrics["turnover_pct"]),
            "—",
            ("Kritik" if metrics["turnover_pct"] >= 30 else
             "İzlemeli" if metrics["turnover_pct"] >= 15 else "Sağlıklı"),
        ],
        [
            "Paket başı maliyet",
            _money(metrics["cost_per_package"]) + " ₺",
            "—",
            ("Yüksek" if metrics["cost_per_package"] > metrics["ecosystem_cpp"] * 1.1
             else "Düşük" if metrics["cost_per_package"] < metrics["ecosystem_cpp"] * 0.9
             else "Normal"),
        ],
    ]
    ...
    # 1. satır (paket değişimi) — renk
    style.add("TEXTCOLOR", (3, 1), (3, 1), growth_color)
    style.add("FONT", (3, 1), (3, 1), FONT_BOLD, 9)
    # turnover badge
    tov = metrics["turnover_pct"]
    tov_color = DANGER if tov >= 30 else AMBER if tov >= 15 else SUCCESS
    style.add("TEXTCOLOR", (3, 3), (3, 3), tov_color)
    style.add("FONT", (3, 3), (3, 3), FONT_BOLD, 9)
    # CPP badge
    cpp_text = rows[4][3]
    cpp_color = DANGER if cpp_text == "Yüksek" else SUCCESS if cpp_text == "Düşük" else TEXT_2
    style.add("TEXTCOLOR", (3, 4), (3, 4), cpp_color)
    style.add("FONT", (3, 4), (3, 4), FONT_BOLD, 9)
...
def _make_ai_commentary(
    ...
    """
    flow: list = []
    flow.append(Paragraph("AI YORUM · ÇAT KAPINDA EKOSİSTEM BENCHMARK'I",
                          styles["section_eyebrow"]))
    flow.append(Paragraph(
        "Aşağıdaki yorum, restoranın metriklerinin Çat Kapında ekosistemindeki "
        "diğer restoranlarla karşılaştırılması üzerine Claude AI tarafından "
        "üretilmiştir. Tüm sayılar gerçek veriden gelir.",
        styles["small"]
    ))
    flow.append(Spacer(1, 8))

    if not commentary:
        # AI yoksa sessizce boş bir uyarı kutusu
        warn = Table(
            [[Paragraph(
                "AI yorumu bu rapor için üretilemedi (API kullanılamıyor). "
                "Yukarıdaki tablo verileri yine tüm gerçek sayılarla doludur.",
                styles["small"]
            )]],
            colWidths=[174 * mm],
        )
...
def _make_footer(restaurant: dict, period: str, styles: dict) -> Table:
    ...
    doc_no = f"RR-{period.replace('-', '')}-{str(restaurant.get('id', 0)).zfill(4)}"
    foot = (
        f"Bu rapor <b>{period_label}</b> dönemi için <b>{restaurant.get('brand')}"
        f"{(' · ' + restaurant.get('branch')) if restaurant.get('branch') else ''}</b> "
        f"restoranının Çat Kapında ekosistemindeki performansını özetler. "
        f"Tüm sayılar puantaj kayıtları, restoran tarifeleri ve bordro verisi "
        f"üzerinden hesaplanmıştır. AI yorumu Claude tarafından üretilmiş olup "
        f"hiçbir sayı uydurulmamıştır."
    )
    ...
    t.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), CREAM_50),
        ("LINEABOVE", (0, 0), (-1, 0), 0.4, CREAM_300),
        ("LEFTPADDING", (0, 0), (-1, -1), 12),
        ("RIGHTPADDING", (0, 0), (-1, -1), 12),
