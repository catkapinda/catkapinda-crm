...
_SYNC_MOBILE_AUTH_USERS: Callable[[Any], None] | None = None
...
_SQLITE_SCHEMA = """
    ...
    CREATE TABLE IF NOT EXISTS box_returns (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        personnel_id INTEGER NOT NULL,
        item_name TEXT NOT NULL DEFAULT 'Box',
...
def _auth_phone_codes_create_sql(backend: str) -> str:
    if backend == "postgres":
        return """
        CREATE TABLE IF NOT EXISTS auth_phone_codes (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
def configure_bootstrap_engine(
    *,
    ...
    sync_mobile_auth_users_fn: Callable[[Any], None],
    ...
    global _SYNC_MOBILE_AUTH_USERS
    ...
    _SYNC_MOBILE_AUTH_USERS = sync_mobile_auth_users_fn
...
def _run_auth_phone_login_migration(conn: Any) -> None:
    auth_user_cols = get_table_columns(conn, "auth_users")
    if "phone" not in auth_user_cols:
        conn.execute("ALTER TABLE auth_users ADD COLUMN phone TEXT")
    conn.commit()
...
def _run_auth_phone_code_login_migration(conn: Any) -> None:
    conn.execute(_auth_phone_codes_create_sql(conn.backend))
    conn.commit()
...
def get_registered_migrations() -> list[MigrationStep]:
    return [
        ...
        MigrationStep(
            version="2026-03-24-auth-phone-login",
            apply_fn=_run_auth_phone_login_migration,
            description="Telefon numarasıyla giriş için auth kullanıcı telefon kolonunu ekle",
        ),
        MigrationStep(
            version="2026-03-24-auth-phone-codes",
            apply_fn=_run_auth_phone_code_login_migration,
            description="Telefonla SMS giriş kodları için auth_phone_codes tablosunu ekle",
        ),
    ]
