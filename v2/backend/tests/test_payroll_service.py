...
def test_payroll_dashboard_uses_30_day_basis_for_mid_month_motor_rental_start():
    raw_conn = sqlite3.connect(":memory:")
    raw_conn.row_factory = sqlite3.Row
    raw_conn.executescript(
        """
    ...
    raw_conn.execute(
        ...
        VALUES (1, 'Emirhan Yılmaz', 'CK-M31', 'Kurye', 'Aktif', 'standard_courier', 0, '2026-03-09', 'Çat Kapında', 'Evet', 'Hayır', 13000)
        """
    )
    raw_conn.execute("INSERT INTO restaurants (id, brand, branch) VALUES (10, 'Burger@', 'Kavacık')")
    raw_conn.execute(
    ...
    assert payload.summary is not None
    assert payload.entries[0].gross_pay == 2500.0
    assert round(payload.entries[0].total_deductions, 2) == 9533.33
    assert round(payload.summary.total_deductions, 2) == 9533.33
